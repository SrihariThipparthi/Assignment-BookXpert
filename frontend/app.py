...
st.set_page_config(
    page_title="Name Matching & Recipe Chatbot",
    page_icon="🤖",
    layout="wide"
)
...
def display_task1():
    st.markdown('<div class="task-header">📝 Task 1: Name Matching System</div>', unsafe_allow_html=True)
    st.write("Find similar names from our dataset using advanced fuzzy matching and semantic similarity.")
        ...
        with st.spinner("Searching for similar names..."):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Additional info
                    st.success(f"Found {len(data['all_matches'])} matching names for '{name_input}'")
...
def display_task2():
    ...
    st.markdown('<div class="task-header">🍳 Task 2: Recipe Chatbot</div>', unsafe_allow_html=True)
    ...
    # Sample ingredients
    st.info("**Try these examples:** egg, onions | chicken, rice | tomato, pasta | potato, onion")
    ...
    if recipe_button and ingredients_input:
        with st.spinner("Cooking up recipe suggestions..."):
            try:
                ...
                st.error("Request timeout. The LLM might be taking too long. Please try again.")
            except Exception as e:
                st.error(f"An error occurred: {str(e)}")
...
def main():
    ...
    # Header
    st.markdown('<div class="main-header"> Name Matching & Recipe Chatbot System</div>', unsafe_allow_html=True)
    ...
    task_choice = st.sidebar.radio(
        "Tasks:",
        ["Task 1: Name Matching", "Task 2: Recipe Chatbot"],
        index=0
    )
    ...
    **Task 2:** Suggests recipes based on your ingredients using either a local LLM (Ollama) or a curated recipe database.
    ...
    <div style="text-align: center; color: #666; padding: 2rem;">
        <p>Built with FastAPI & Streamlit | Powered by Machine Learning </p>
    </div>
    """, unsafe_allow_html=True)
