...
class NameMatcher:
    def __init__(self):
        ...
        self.name_embeddings = self.model.encode(self.names, convert_to_tensor=True, show_progress_bar=False)
    ...
    def _get_default_names(self):
        return [
            "Geetha", "Geeta", "Gita", "Gitu", "Gitanjali",
            "Priya", "Priyanka", "Priyam", "Priyanshu",
            "Amit", "Amitabh", "Amith", "Amita",
            "Rajesh", "Raja", "Raju", "Rajeev", "Rajiv",
            "Suresh", "Suri", "Suraj", "Surya",
            "Mohammed", "Mohammad", "Muhammad", "Muhammed",
            "Aisha", "Ayesha", "Aysha", "Aiesha",
            "Krishna", "Krish", "Krishnan", "Karthik", "Kartik",
            "Lakshmi", "Laxmi", "Lakshman",
            "Deepak", "Dipak", "Deepika", "Dipika",
            "Sandeep", "Sandip", "Sanjay", "Sanjiv",
            "Ramesh", "Ram", "Raman", "Rama"
        ]
    ...
    def find_similar_names(self, input_name, top_k=5):
        ...
        semantic_scores = util.cos_sim(input_embedding, self.name_embeddings)[0].cpu().numpy()
...
if __name__ == "__main__":
    ...
    for test_name in test_names:
        ...
        for i, match in enumerate(results['all_matches'][:5], 1):
            logger.info(f"{i}. {match['name']:<15} - Score: {match['score']:.3f} "
