...
class RecipeBot:
    ...
    def generate_recipe(self, ingredients: str, use_fallback: bool = True) -> Dict[str, str]:
        try:
...
{ingredients_text} [/INST]"""
            ...
            if not self._is_valid_recipe(recipe_text):
                logger.info("Generated response doesn't look like a recipe, using fallback...")
                if use_fallback:
    ...
    def _is_valid_recipe(self, text: str) -> bool:
        ...
        cooking_words = ['cook', 'heat', 'add', 'mix', 'stir', 'serve', 'bake', 'fry', 'boil']
        has_cooking_actions = sum(1 for word in cooking_words if word in text_lower) >= 2
